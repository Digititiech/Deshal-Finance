...
export const ExpensesModule: React.FC<ExpensesModuleProps> = ({
  ...
  useEffect(() => {
    if (viewingPaymentVoucher) {
      const matchedVendor = vendors.find(v => v.name.toLowerCase() === viewingPaymentVoucher.entity.toLowerCase() || (v.nameAr && v.nameAr === viewingPaymentVoucher.entityAr));
      const matchedCust = customers.find(c => c.name.toLowerCase() === viewingPaymentVoucher.entity.toLowerCase() || (c.nameAr && c.nameAr === viewingPaymentVoucher.entityAr));
      setEmailTarget(matchedVendor ? matchedVendor.contactEmail : (matchedCust ? matchedCust.contactEmail : ''));
      setShowEmailInput(false);
      setEmailError('');
      setEmailSuccess('');
    }
  }, [viewingPaymentVoucher, vendors, customers]);
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      const voucherNo = `PV-${viewingPaymentVoucher!.id.slice(-6).toUpperCase()}`;
      ...
      const mailSubject = lang === 'ar' 
        ? `سند صرف نفقات رسمي: ${voucherNo}` 
        : `Official Expense Payment Voucher: ${voucherNo}`;
      ...
      const mailHtml = `
        <div style="font-family: sans-serif; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05);">
          <div style="background: #e11d48; padding: 24px; color: white; text-align: center;">
            <h2 style="margin: 0; font-size: 20px;">${lang === 'ar' ? 'سند صرف نفقات معتمد' : 'Expense Payment Voucher'}</h2>
          ...
          <div style="padding: 24px; background: #fafafa;">
            <table style="width: 100%; border-collapse: collapse; font-size: 12px; margin-bottom: 20px;">
              ...
              <tr>
                <td style="padding: 6px 0; color: #64748b;"><b>${lang === 'ar' ? 'الجهة المستفيدة:' : 'Disbursed To (Entity):'}</b></td>
                <td style="padding: 6px 0; text-align: right; color: #1e293b;"><b>${lang === 'ar' ? viewingPaymentVoucher!.entityAr : viewingPaymentVoucher!.entity}</b></td>
              </tr>
              <tr>
              ...
              <tr>
                <td style="padding: 6px 0; color: #64748b;"><b>${lang === 'ar' ? 'تصنيف النفقة:' : 'Category classification:'}</b></td>
            ...
            <div style="background: white; border: 1px solid #e2e8f0; border-radius: 12px; padding: 16px; margin-bottom: 20px; text-align: right;">
              <div>
                <span style="font-size: 10px; color: #94a3b8; font-weight: bold; display: block; text-transform: uppercase;">${lang === 'ar' ? 'ملاحظات الصرف والبيان' : 'DISBURSEMENT ANNOTATION'}</span>
                <p style="font-size: 11px; color: #475569; margin: 4px 0 0 0; font-style: italic;">
                  ${(lang === 'ar' ? viewingPaymentVoucher!.descriptionAr : viewingPaymentVoucher!.description) || (lang === 'ar' ? 'تم قيد وتسجيل مصروفات الحساب الجاري بنجاح' : 'Expense disbursement cleared and recorded.')}
                </p>
               </div>
               <div style="text-align: right; margin-top: 12px; border-top: 1px solid #e2e8f0; padding-top: 12px;">
                 <span style="font-size: 10px; color: #94a3b8; font-weight: bold; display: block;">${lang === 'ar' ? 'القيمة الإجمالية المصروفة' : 'Voucher Disbursed Value'}</span>
            ...
            <div style="border-top: 1px solid #e2e8f0; padding-top: 12px; font-size: 9px; color: #94a3b8; font-style: italic;">
              ...
              ${lang === 'ar' 
                ...
                : 'This payment voucher was reviewed and verified in compliance with local accounting practices.'}
            </div>
          </div>
      ...
      setEmailSuccess(lang === 'ar' ? 'تم إرسال السند بنجاح إلى البريد الإلكتروني!' : 'Payment voucher dispatched successfully via email!');
      setTimeout(() => {
        setShowEmailInput(false);
        setEmailSuccess('');
      }, 3000);
