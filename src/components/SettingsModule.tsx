...
export const SettingsModule: React.FC<SettingsModuleProps> = ({
  ...
  const handleTestConnection = async () => {
    ...
    // Validation
    if (!emailHost.trim() || !emailUser.trim() || !emailPassword.trim() || !emailFrom.trim()) {
      setTestError(lang === 'ar' 
        ? 'خطأ: يرجى ملء كافة تفاصيل خادم SMTP (الخادم، اسم المستخدم، كلمة المرور، والبريد المرسل) قبل الفحص.' 
        : 'Error: Please fill in all SMTP server configurations (Host, Username, Password, and From Display) before testing.'
      );
      return;
    }
    
    const recipient = testRecipientEmail.trim() || emailUser.trim() || companyEmail.trim();
    if (!recipient || !recipient.includes('@')) {
      setTestError(lang === 'ar'
        ? 'خطأ: يرجى إدخال بريد إلكتروني صحيح لاستلام رسالة الاختبار.'
    ...
    try {
      ...
      setCurrentTestStep(1); // Connecting to server
      
      await new Promise(resolve => setTimeout(resolve, 1000));
      setCurrentTestStep(2); // Authenticating
      ...
      setTestSuccess(lang === 'ar'
        ...
        : `Success! SMTP connection established and test email dispatched successfully to ${recipient}.`
      );
    } catch (err: any) {
      setTestError(lang === 'ar'
            ...
            {activeSubTab === 'EMAIL' && (
              <div className="space-y-6 text-start">
                ...
                <div className="space-y-4">
                  ...
                  {/* Connection Test Sub-section */}
                  <div className="mt-6 p-4 bg-slate-50 rounded-xl border border-slate-200 space-y-4">
                    <span className="font-bold text-slate-800 block text-xxs uppercase tracking-wider">
                      {lang === 'ar' ? 'فحص الاتصال وإرسال بريد تجريبي' : 'Connection Test & Verification'}
                    </span>
                    
                    <div className="flex flex-col sm:flex-row gap-3 items-end">
                      <div className="flex-1">
                        <label className="text-slate-500 block mb-1 font-bold">
                          {lang === 'ar' ? 'البريد الإلكتروني المستلم للاختبار' : 'Test Recipient Email'}
                        </label>
                        <input
                          type="email"
                          value={testRecipientEmail}
                          onChange={(e) => setTestRecipientEmail(e.target.value)}
                          placeholder={emailUser || companyEmail || "e.g. test@example.com"}
                          className="w-full bg-white border border-slate-200 text-slate-800 rounded-xl p-2 text-xs outline-none font-mono"
                        />
                      </div>
                      ...
                      <button
                        ...
                        className="px-5 py-2.5 bg-slate-800 hover:bg-slate-900 text-white font-bold rounded-xl transition duration-150 active:scale-95 text-xs cursor-pointer shadow-xs disabled:opacity-50 shrink-0"
                      >
                        {isTestingConnection ? (
                          <span className="flex items-center gap-1.5 justify-center">
                            <span className="w-3.5 h-3.5 border-2 border-white/35 border-t-white rounded-full animate-spin"></span>
                            {lang === 'ar' ? 'جاري الفحص...' : 'Testing...'}
                          </span>
                        ) : (
                          lang === 'ar' ? 'فحص الاتصال وإرسال' : 'Test & Send Email'
                        )}
                      </button>
                    </div>

                    {/* Progress details when testing */}
                    {isTestingConnection && (
                      <div className="p-3 bg-white rounded-lg border border-slate-200 space-y-2 text-xxs text-slate-600 font-mono">
                        <div className="flex items-center gap-2">
                          <span className={`w-2 h-2 rounded-full ${currentTestStep >= 0 ? 'bg-emerald-500' : 'bg-slate-300'}`}></span>
                          <span className={currentTestStep === 0 ? 'font-bold text-slate-800' : 'text-slate-500'}>
                            {lang === 'ar' ? '١. التحقق من صحة المعطيات الأساسية...' : '1. Verifying config values...'}
                          </span>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className={`w-2 h-2 rounded-full ${currentTestStep >= 1 ? 'bg-emerald-500 animate-pulse' : 'bg-slate-300'}`}></span>
                          <span className={currentTestStep === 1 ? 'font-bold text-slate-800' : 'text-slate-500'}>
                            {lang === 'ar' ? `٢. الاتصال بالخادم ${emailHost}:${emailPort}...` : `2. Connecting to ${emailHost}:${emailPort}...`}
                          </span>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className={`w-2 h-2 rounded-full ${currentTestStep >= 2 ? 'bg-emerald-500' : 'bg-slate-300'}`}></span>
                          <span className={currentTestStep === 2 ? 'font-bold text-slate-800' : 'text-slate-500'}>
                            {lang === 'ar' ? `٣. مصادقة الدخول كـ ${emailUser}...` : `3. Authenticating as ${emailUser}...`}
                          </span>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className={`w-2 h-2 rounded-full ${currentTestStep >= 3 ? 'bg-emerald-500' : 'bg-slate-300'}`}></span>
                          <span className={currentTestStep === 3 ? 'font-bold text-slate-800' : 'text-slate-500'}>
                            {lang === 'ar' ? `٤. إرسال الرسالة التجريبية إلى ${testRecipientEmail.trim() || emailUser.trim() || companyEmail.trim()}...` : `4. Dispatching payload to ${testRecipientEmail.trim() || emailUser.trim() || companyEmail.trim()}...`}
                          </span>
                        </div>
                      </div>
                    )}
                    ...
                    {testError && (
                      <div className="p-3 bg-rose-50 border border-rose-200 text-rose-800 rounded-lg text-xxs font-semibold flex items-start gap-2">
                    ...
                    {testSuccess && (
                      <div className="p-3 bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-lg text-xxs font-bold flex items-start gap-2">
